 ...
 private:
  ...
  Expr VisitExpr_(const FunctionNode* function) final {
    ...
    if (composite_name.defined()) {
      ...
      if (name.find("headsail") == std::string::npos) {
        return func;
      }
    }
    auto compiler_name = func->GetAttr<String>(::tvm::relay::attr::kCompiler);
    if (compiler_name.defined() && compiler_name != "headsail") {
      return func;
    }
...
TVM_REGISTER_GLOBAL("relay.ext.headail.transform.ExtractConstantsFromPartitionedFunction")
