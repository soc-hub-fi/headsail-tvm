 ...
 private:
  ...
  Expr Rewrite_(const CallNode* call, const Expr& post) final {
    ...
    if (call->op.as<FunctionNode>()) {
      ...
      if (func_name.defined() && func_name == "headsail-nn.qnn_conv2d") {
...
transform::Pass GenerateHeadsailonstants() {
  runtime::TypedPackedFunc<IRModule(IRModule, transform::PassContext)> pass_func =
      [=](IRModule m, transform::PassContext pc) { return GenerateConstants(m); };
  return tvm::transform::CreateModulePass(pass_func, 0, "GenerateHeadsailConstants", {});
}

TVM_REGISTER_GLOBAL("relay.ext.headsail.transform.GenerateHeadsailConstants")
    .set_body_typed(GenerateHeadsailConstants);
